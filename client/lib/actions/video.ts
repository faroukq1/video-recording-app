...
const ACCESS_KEY = {
  streamAcessKey: getEnv('BUNNY_STREAM_ACCESS_KEY'),
...
const validateWithArcjet = async (fingerprint: string) => {
  ...
  if (decision.isDenied()) throw new Error('Rate limite exceeded');
};
...
export const getVideoUploadUrl = withErrorHandling(async () => {
  ...
  return {
    ...
    uploadUrl,
    accessKey: ACCESS_KEY.streamAcessKey,
  };
});
...
export const getThumbnailUploadUrl = withErrorHandling(
  async (videoId: string) => {
    ...
    return {
      uploadUrl,
      cdnUrl,
      accessKey: ACCESS_KEY.storageAcessKey,
    };
  }
);
...
// /?searchQuery=test&pageNumber=2 (exmaple)
